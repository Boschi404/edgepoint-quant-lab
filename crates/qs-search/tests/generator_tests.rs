...
fn grid_generation_respects_budget() {
    let space = ParameterSpace {
        ...
        neighborhood: NeighborhoodDefinition {
            metric: "normalized_mixed".into(),
            weights: BTreeMap::new(),
        },
    };
    let candidates = match generate_grid(&space, GenerationBudget { max_candidates: 5 }) {
        Ok(value) => value,
        Err(err) => panic!("grid should generate: {err}"),
    };
...
fn neighborhood_distance_is_zero_for_same_candidate() {
    ...
    let mut candidates = match generate_grid(&space, GenerationBudget { max_candidates: 1 }) {
        Ok(value) => value,
        Err(err) => panic!("grid: {err}"),
    };
    let candidate = candidates.remove(0);
    let distance = match DefaultNeighborhood.distance(&candidate, &candidate, &space) {
        Ok(value) => value,
        Err(err) => panic!("distance: {err}"),
    };
...
fn budgeted_generation_uses_sparse_source_when_space_exceeds_budget() {
    ...
    let candidates =
        match qs_search::generate_budgeted(&space, GenerationBudget { max_candidates: 10 }, 42) {
            Ok(value) => value,
    ...
    assert!(matches!(
        candidates[0].source,
        ParameterSetSource::RandomSparse
    ));
}
...
fn intensification_plan_selects_top_candidate() {
    ...
    let evaluated = candidates
        ...
        .take(2)
        .enumerate()
        .map(|(idx, candidate)| EvaluationResult {
            run_id: RunId("r".into()),
            strategy_id: StrategyId("s".into()),
            ...
            metrics: MetricBundle {
                classical: ClassicalMetrics {
                    expectancy: idx as f64,
                    profit_factor: 1.0,
                    ..ClassicalMetrics::default()
                },
        ...
        .collect::<Vec<_>>();
    let plan = match qs_search::build_intensification_plan_from_candidates(
        &space,
        &evaluated,
        &candidates,
        &qs_search::SearchPlanConfig::default(),
    ) {
        Ok(value) => value,
