...
impl Default for SearchPlanConfig {
    fn default() -> Self {
        Self {
            top_n: 5,
            intensification_radius: 0.20,
            intensification_budget_per_top: 8,
...
pub fn build_intensification_plan(
    ...
    let mut ranked = evaluated
        .iter()
        .filter(|result| result.strategy_id == space.strategy_id)
        .collect::<Vec<_>>();
    ranked.sort_by(|a, b| {
    ...
    for result in top {
        let Some(center) = lookup.get(&result.parameter_set_id) else {
            continue;
        };
        for candidate in neighborhood.neighbors(
            center,
            config.intensification_radius,
            config.intensification_budget_per_top,
            space,
        )? {
            if evaluated_ids.contains(&candidate.id) || !generated_ids.insert(candidate.id.clone())
            {
                continue;
            }
    ...
    let baseline = crate::generate_budgeted(
        space,
        GenerationBudget {
            max_candidates: config
                .completion_budget
                .saturating_mul(4)
                .max(config.completion_budget),
        },
        17,
    )?;
    for candidate in baseline {
        if completion_candidates.len() >= config.completion_budget {
            break;
        }
        if evaluated_ids.contains(&candidate.id) || generated_ids.contains(&candidate.id) {
            continue;
        }
...
pub fn robust_score(result: &EvaluationResult) -> f64 {
    ...
    let pf = if c.profit_factor.is_finite() {
        c.profit_factor.min(5.0) / 5.0
    } else {
        0.0
    };
...
fn collect_parameter_sets(
    evaluated: &[EvaluationResult],
) -> BTreeMap<ParameterSetId, ParameterSet> {
    let mut out = BTreeMap::new();
    for result in evaluated {
        out.entry(result.parameter_set_id.clone())
            .or_insert_with(|| ParameterSet {
...
pub fn build_intensification_plan_from_candidates(
    ...
    for parameter_set_id in &selected_top {
        let Some(center) = candidate_lookup.get(parameter_set_id) else {
            continue;
        };
        for candidate in neighborhood.neighbors(
            center,
    ...
    let completion_candidates = candidates
        .iter()
        .filter(|candidate| {
            !evaluated_ids.contains(&candidate.id) && !generated_ids.contains(&candidate.id)
        })
