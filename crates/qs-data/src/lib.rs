...
impl PipelineComponent for DataIngestionComponent {
    fn id(&self) -> ComponentId {
        ComponentId("DataIngestion".into())
    }
    ...
    fn output_contract(&self) -> Vec<DataContract> {
        vec![DataContract::RawDatasetReference]
    }
    ...
    async fn execute(&self, ctx: &mut PipelineContext) -> Result<ComponentOutcome, PipelineError> {
        let Some(value) = ctx.bag.get("dataset_configs").cloned() else {
            ...
            return Ok(ComponentOutcome {
                message: "no dataset configs supplied; raw dataset list is empty".into(),
            });
        };
        let configs: Vec<ConfiguredDataset> = serde_json::from_value(value).map_err(|e| {
            PipelineError::Data(DataError::Message {
                code: "DATASET_CONFIG_PARSE".into(),
                message: e.to_string(),
                retryable: false,
            })
        })?;
        ...
        for config in configs {
            if !csv.detect(&config.source)? {
                return Err(PipelineError::Data(DataError::Message {
                    code: "NO_ADAPTER".into(),
                    message: format!("no adapter for {}", config.source.uri),
                    retryable: false,
                }));
            }
            let schema = csv.read_schema(&config.source)?;
            let mut records = Vec::new();
            for record in csv.read_records(&config.source)? {
                records.push(record?);
            }
            raw_datasets.push(RawDataset {
        ...
        ctx.bag.insert(
            "raw_datasets".into(),
            serde_json::to_value(raw_datasets).map_err(|e| {
                PipelineError::Data(DataError::Message {
                    code: "RAW_DATASETS_SERIALIZE".into(),
                    message: e.to_string(),
                    retryable: false,
                })
            })?,
        ...
        Ok(ComponentOutcome {
            message: "raw datasets ingested".into(),
        })
    }
}
...
impl PipelineComponent for DataNormalizerComponent {
    fn id(&self) -> ComponentId {
        ComponentId("DataNormalizer".into())
    }
    fn name(&self) -> &'static str {
        "DataNormalizer"
    }
    ...
    fn output_contract(&self) -> Vec<DataContract> {
        vec![DataContract::NormalizedDataset]
    }
    fn failure_policy(&self) -> FailurePolicy {
        FailurePolicy::AbortRun
    }
    async fn execute(&self, ctx: &mut PipelineContext) -> Result<ComponentOutcome, PipelineError> {
        let configs_value = match ctx.bag.get("dataset_configs").cloned() {
            Some(value) => value,
            None => serde_json::json!([]),
        };
        let raw_value = match ctx.bag.get("raw_datasets").cloned() {
            Some(value) => value,
            None => serde_json::json!([]),
        };
        let configs: Vec<ConfiguredDataset> =
            serde_json::from_value(configs_value).map_err(|e| {
                PipelineError::Data(DataError::Message {
                    code: "DATASET_CONFIG_PARSE".into(),
                    message: e.to_string(),
                    retryable: false,
        ...
        let raws: Vec<RawDataset> = serde_json::from_value(raw_value).map_err(|e| {
            PipelineError::Data(DataError::Message {
                code: "RAW_DATASETS_PARSE".into(),
                message: e.to_string(),
                retryable: false,
            })
        })?;
        for (config, raw) in configs.into_iter().zip(raws.into_iter()) {
            let normalizer = OhlcvJsonNormalizer {
        ...
        Ok(ComponentOutcome {
            message: format!("{} datasets normalized", ctx.datasets.len()),
        })
    }
}

pub struct DataQualityGateComponent {
...
impl PipelineComponent for DataQualityGateComponent {
    fn id(&self) -> ComponentId {
        ComponentId("DataQualityGate".into())
    }
    fn name(&self) -> &'static str {
        "DataQualityGate"
    }
    ...
    async fn execute(&self, ctx: &mut PipelineContext) -> Result<ComponentOutcome, PipelineError> {
        ...
        Ok(ComponentOutcome {
            message: "quality checks completed".into(),
        })
    }
}
...
pub fn run_quality_checks(
    ...
    for (idx, bar) in dataset.bars.iter().enumerate() {
        if let Some(previous) = prev_timestamp {
            if bar.timestamp <= previous {
                failed = true;
                checks.push(DataQualityCheckResult {
                    check_name: "timestamp_monotonic".into(),
                    passed: false,
                    severity: QualitySeverity::Error,
                    message: "non-monotonic timestamp".into(),
                    affected_rows: vec![idx],
                });
            }
            ...
            && bar.low <= bar.close)
        {
            failed = true;
            checks.push(DataQualityCheckResult {
                check_name: "ohlc_consistency".into(),
                passed: false,
                severity: QualitySeverity::Error,
                message: "OHLC invariant violated".into(),
                affected_rows: vec![idx],
            });
        }
        ...
        if let Some(spread) = bar.spread {
            if spread < 0.0 {
                failed = true;
                checks.push(DataQualityCheckResult {
                    check_name: "negative_spread".into(),
                    passed: false,
                    severity: QualitySeverity::Error,
                    message: "negative spread".into(),
                    affected_rows: vec![idx],
                });
            }
        }
    ...
    if !gap_rows.is_empty() {
        checks.push(DataQualityCheckResult {
            check_name: "gap_detection".into(),
            passed: false,
            severity: QualitySeverity::Warning,
            message: "detected timestamp gaps larger than configured timeframe tolerance".into(),
            affected_rows: gap_rows,
        });
    }
    if !outlier_rows.is_empty() {
        checks.push(DataQualityCheckResult {
            check_name: "outlier_detection".into(),
            passed: false,
            severity: QualitySeverity::Warning,
            message: "detected non-positive OHLC values".into(),
            affected_rows: outlier_rows,
        });
    }
    if checks.is_empty() {
        checks.push(DataQualityCheckResult {
            check_name: "basic_checks".into(),
            passed: true,
            severity: QualitySeverity::Info,
            message: "basic checks passed".into(),
            affected_rows: vec![],
        });
    }
    if failed && matches!(policy, DataQualityPolicy::Block) {
        return Err(DataError::Message {
            code: "DATA_QUALITY_BLOCKED".into(),
            message: "dataset failed quality gate".into(),
            retryable: false,
        });
    }
    let status = if failed {
        ...
        DataQualityStatus::PassedWithWarnings
    } else {
