...
impl RawDataAdapter for CsvRawDataAdapter {
    fn adapter_id(&self) -> String { "csv".into() }

    fn detect(&self, source: &DataSource) -> Result<bool, DataError> {
        if matches!(source.format_hint.as_deref(), Some("csv")) { return Ok(true); }
    ...
    fn read_schema(&self, source: &DataSource) -> Result<RawSchema, DataError> {
        ...
        Ok(RawSchema { fields: headers.iter().map(str::to_owned).collect() })
    }
    ...
    fn read_records(&self, source: &DataSource) -> Result<Box<dyn Iterator<Item = Result<RawRecord, DataError>>>, DataError> {
        ...
        for result in reader.records() {
            ...
            for (idx, value) in record.iter().enumerate() {
                let Some(key) = headers.get(idx) else { continue; };
                map.insert(key.to_owned(), serde_json::Value::String(value.to_owned()));
            }
            rows.push(Ok(RawRecord { fields: serde_json::Value::Object(map) }));
        }
...
fn file_path_from_uri(uri: &str) -> &Path {
    let path = match uri.strip_prefix("file://") { Some(value) => value, None => uri };
    Path::new(path)
}

fn data_err<E: std::fmt::Display>(code: &'static str) -> impl Fn(E) -> DataError {
    move |e| DataError::Message { code: code.into(), message: e.to_string(), retryable: true }
}
