...
impl LiveExportWriter {
    ...
    pub fn write_pack(
        ...
        selected: &SelectedParameterExport,
        ...
        atomic_write_json(&dir.join("manifest.json"), manifest).map_err(|e| {
            ExportError::Message {
                code: "EXPORT_MANIFEST".into(),
                message: e.to_string(),
                retryable: true,
            }
        })?;
        atomic_write_json(&dir.join("selected_parameters.json"), selected).map_err(|e| {
            ExportError::Message {
                code: "EXPORT_SELECTED".into(),
                message: e.to_string(),
                retryable: true,
            }
        })?;
        atomic_write_json(
            &dir.join("python_bot_pack").join("strategy_config.json"),
            selected,
        )
        .map_err(|e| ExportError::Message {
            code: "EXPORT_PY_CONFIG".into(),
            message: e.to_string(),
            retryable: true,
        })?;
...
fn write_mt5_set(path: &Path, selected: &SelectedParameterExport) -> Result<(), ExportError> {
    ...
    for item in &selected.selected {
        writeln!(file, "; strategy_id={}", item.strategy_id.0)
            .map_err(export_err("EXPORT_MT5_WRITE"))?;
        writeln!(file, "; parameter_set_id={}", item.parameter_set_id.0)
            .map_err(export_err("EXPORT_MT5_WRITE"))?;
        for (key, value) in &item.parameters {
            writeln!(file, "{}={}", key, param_to_mt5(value))
                .map_err(export_err("EXPORT_MT5_WRITE"))?;
        }
    }
