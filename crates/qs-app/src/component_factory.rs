...
pub fn build_default_components(registry: StaticStrategyRegistry) -> Vec<Box<dyn PipelineComponent>> {
    vec![
        Box::new(qs_data::DataIngestionComponent),
        Box::new(qs_data::DataNormalizerComponent),
        Box::new(qs_data::DataQualityGateComponent { policy: DataQualityPolicy::Block }),
        Box::new(qs_search::ParameterGeneratorComponent),
        Box::new(qs_search::ParameterSearchComponent),
        Box::new(qs_backtest::StrategyRunnerComponent::new(registry, qs_backtest::ExecutionModel::default())),
