...
pub fn walk_forward_report(result: &EvaluationResult, folds: usize) -> WalkForwardReport {
    ...
    for (idx, slice) in result.trades.chunks(chunk).enumerate() {
        ...
        let start_time = match slice.first() {
            Some(trade) => trade.entry_time,
            None => 0,
        };
        let end_time = match slice.last() {
            Some(trade) => trade.exit_time,
            None => 0,
        };
        out.push(WalkForwardFold {
            ...
            start_time,
            end_time,
            trade_count: slice.len(),
            total_r,
            average_r,
        });
    }
    let positive = out.iter().filter(|fold| fold.total_r > 0.0).count();
    let consistency_ratio = if out.is_empty() {
        0.0
    } else {
        positive as f64 / out.len() as f64
    };
    WalkForwardReport {
        schema_version: 1,
        strategy_id: result.strategy_id.clone(),
...
pub fn monte_carlo_report(
    ...
    totals.sort_by(|a, b| match a.partial_cmp(b) {
...
pub fn sensitivity_report(result: &EvaluationResult) -> SensitivityReport {
    ...
    let mean_abs = if returns.is_empty() {
        ...
        returns.iter().map(|value| value.abs()).sum::<f64>() / returns.len() as f64
    };
...
pub fn regime_report(result: &EvaluationResult) -> RegimeReport {
    ...
    for trade in &result.trades {
        if trade.r_multiple >= 0.0 {
            positive.push(trade.r_multiple);
        } else {
            negative.push(trade.r_multiple);
        }
    }
    let buckets = vec![
        bucket_report("positive_trades", &positive),
        bucket_report("negative_trades", &negative),
    ];
    ...
    RegimeReport {
        ...
        parameter_set_id: result.parameter_set_id.clone(),
        buckets,
        inter_bucket_variance: sample_variance(&averages),
    }
}

pub fn execution_stress_report(
    result: &EvaluationResult,
    slippage_r_per_trade: f64,
    ...
    ExecutionStressReport {
        ...
        base_total_r,
        stressed_total_r,
        stress_loss: base_total_r - stressed_total_r,
    }
}

pub fn parameter_decay_report(result: &EvaluationResult) -> ParameterDecayReport {
    let mid = result.trades.len() / 2;
    let first_half_total_r = result.trades[..mid]
        .iter()
        .map(|trade| trade.r_multiple)
        .sum::<f64>();
    let second_half_total_r = result.trades[mid..]
        .iter()
        .map(|trade| trade.r_multiple)
        .sum::<f64>();
    let decay_ratio = if first_half_total_r.abs() < f64::EPSILON {
        None
    } else {
        Some(second_half_total_r / first_half_total_r)
    };
    ParameterDecayReport {
        schema_version: 1,
        strategy_id: result.strategy_id.clone(),
        parameter_set_id: result.parameter_set_id.clone(),
        first_half_total_r,
...
fn bucket_report(name: &str, values: &[f64]) -> RegimeBucketReport {
    ...
    let average_r = if values.is_empty() {
        0.0
    } else {
        total_r / values.len() as f64
    };
    RegimeBucketReport {
        bucket: name.into(),
