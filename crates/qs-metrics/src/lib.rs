...
pub fn classical_metrics(trades: &[Trade], equity: &[EquityPoint]) -> ClassicalMetrics {
    ...
    let average_r = match mean(&returns) { Some(value) => value, None => 0.0 };
    ...
    ClassicalMetrics {
        ...
        winrate: if returns.is_empty() { 0.0 } else { wins as f64 / returns.len() as f64 },
        profit_factor: safe_profit_factor(gross_win, gross_loss),
        max_drawdown,
        sharpe: sharpe_like(&returns),
        sortino: sortino_like(&returns),
        calmar: if max_drawdown == 0.0 { None } else { Some(total_r / max_drawdown) },
        ...
        recovery_factor: if max_drawdown == 0.0 { None } else { Some(total_r / max_drawdown) },
        average_trade_duration_secs: average_duration(trades),
    }
}

pub fn stability_metrics(trades: &[Trade], equity: &[EquityPoint], window: usize) -> StabilityMetrics {
    let returns: Vec<f64> = trades.iter().map(|t| t.r_multiple).collect();
    StabilityMetrics {
...
fn stddev(values: &[f64]) -> Option<f64> { variance(values).map(f64::sqrt) }

fn sharpe_like(values: &[f64]) -> Option<f64> {
    let m = mean(values)?;
    let sd = stddev(values)?;
    if sd == 0.0 { None } else { Some(m / sd * (values.len() as f64).sqrt()) }
}
...
fn sortino_like(values: &[f64]) -> Option<f64> {
    ...
    if dd == 0.0 { None } else { Some(m / dd * (values.len() as f64).sqrt()) }
}

fn safe_profit_factor(gross_win: f64, gross_loss: f64) -> f64 {
    if gross_loss == 0.0 {
        if gross_win > 0.0 { 999.0 } else { 0.0 }
    } else {
...
fn average_duration(trades: &[Trade]) -> Option<f64> {
    if trades.is_empty() {
        ...
        Some(trades.iter().map(|t| (t.exit_time - t.entry_time) as f64).sum::<f64>() / trades.len() as f64)
    }
}

fn rolling_mean_points(trades: &[Trade], window: usize) -> Vec<RollingPoint> {
    if window == 0 || trades.len() < window { return Vec::new(); }
    trades.windows(window).filter_map(|slice| {
        let end = slice.last()?;
        Some(RollingPoint { timestamp: end.exit_time, value: slice.iter().map(|t| t.r_multiple).sum::<f64>() / window as f64 })
    }).collect()
}
...
fn rolling_profit_factor_points(trades: &[Trade], window: usize) -> Vec<RollingPoint> {
    ...
    trades.windows(window).filter_map(|slice| {
        let end = slice.last()?;
        let win: f64 = slice.iter().filter(|t| t.r_multiple > 0.0).map(|t| t.r_multiple).sum();
        let loss: f64 = slice.iter().filter(|t| t.r_multiple < 0.0).map(|t| t.r_multiple.abs()).sum();
        let value = safe_profit_factor(win, loss);
        Some(RollingPoint { timestamp: end.exit_time, value })
    }).collect()
}
...
fn ulcer_index(equity: &[EquityPoint]) -> Option<f64> {
    if equity.is_empty() { return None; }
...
fn z_score_runs(trades: &[Trade]) -> Option<f64> {
    ...
    if wins == 0.0 || losses == 0.0 { return None; }
    let mut runs = 1.0;
    ...
    if variance_runs <= 0.0 { None } else { Some((runs - expected_runs) / variance_runs.sqrt()) }
}
