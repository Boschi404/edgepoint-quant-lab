use qs_backtest::{
    limit_fill, market_fill, normalize_size, round_to_tick, ExecutionConstraints, OrderIntent,
    OrderType,
};
use qs_core::*;
...
fn market_order_fills_on_close_with_slippage() {
    ...
    let fill = match market_fill(&intent, &bar(), &constraints, 0.1, 0.0) {
        Some(value) => value,
        None => panic!("expected fill"),
    };
...
fn limit_order_respects_bar_range() {
    ...
    let intent = OrderIntent {
        timestamp: 1,
        direction: TradeDirection::Long,
        order_type: OrderType::Limit { price: 99.5 },
        requested_size: 1.0,
        tags: BTreeMap::new(),
    };
