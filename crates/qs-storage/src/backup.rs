...
impl RunBackupService {
    ...
    pub fn build_manifest(&self, run_id: &RunId) -> Result<BackupManifest, StorageError> {
        ...
        for subdir in ["metadata", "checkpoints", "results", "artifacts"] {
            ...
            let target = if subdir == "metadata" {
                base.join(format!("{}.json", run_id.0))
            } else {
                base.join(&run_id.0)
            };
    ...
    pub fn write_manifest(&self, run_id: &RunId) -> Result<PathBuf, StorageError> {
        ...
        crate::atomic_write_json(&path, &manifest).map_err(|e| StorageError::Message {
            code: "BACKUP_MANIFEST_WRITE".into(),
            message: e.to_string(),
            retryable: true,
        })?;
        Ok(path)
...
fn collect_files(root: &Path, path: &Path, out: &mut Vec<BackupFile>) -> Result<(), StorageError> {
    ...
    if path.is_file() {
        ...
        let relative = path.strip_prefix(root).map_err(|e| StorageError::Message {
            code: "BACKUP_STRIP_PREFIX".into(),
            message: e.to_string(),
            retryable: false,
        })?;
        out.push(BackupFile {
